...
public partial class MainWindow : Window
{
    ...
    private void ExpandGroup(string name) { foreach (var g in _vm.Groups) { if (g.GroupName == name) { g.IsExpanded = true; break; } } }

    private async void GroupAddBtn_Click(object sender, RoutedEventArgs e) { var g = (TodoGroupViewModel)((Button)sender).DataContext; var dlg = new Views.AddEditDialog(_repo, null, () => Task.CompletedTask, g.GroupName) { Owner = this, Topmost = true }; if (dlg.ShowDialog() == true) { await _vm.LoadAsync(); ExpandGroup(dlg.FinalGroup); } SendToBottom(); }
    ...
    private async void EditBtn_Click(object sender, RoutedEventArgs e) { if (((Button)sender).DataContext is TodoItem item) { var dlg = new Views.AddEditDialog(_repo, item, () => Task.CompletedTask) { Owner = this, Topmost = true }; if (dlg.ShowDialog() == true) { await _vm.LoadAsync(); ExpandGroup(dlg.FinalGroup); } SendToBottom(); } }
    ...
    private async void AddBtn_Click(object sender, RoutedEventArgs e) { var dlg = new Views.AddEditDialog(_repo, null, () => Task.CompletedTask) { Owner = this, Topmost = true }; if (dlg.ShowDialog() == true) { await _vm.LoadAsync(); ExpandGroup(dlg.FinalGroup); } SendToBottom(); }
